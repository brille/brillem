classdef light_python_wrapper < dynamicprops
    ...
    methods(Static, Access=protected)
        function out = parse_args(args, fun_ref)
            ...
            for ii = 1:numel(args)
                if isa(args{ii}, 'brille.light_python_wrapper')
    ...
    methods
        ...
        function varargout = subsref(obj, s)
            ...
            switch s(1).type
                    ...
                    if any(cellfun(@(c) strcmp(s(1).subs, c), obj.overrides))
                        ...
                        try
                            ...
                            if ~strcmp(s(end).subs, 'pyobj')
                                try %#ok<TRYNC>
                                    varargout = brille.p2m(varargout);
                                end
                            end
                        catch ME
                ...
                case '()'
                    args = brille.light_python_wrapper.parse_args(s(1).subs, obj.pyobj);
...
% This allows the overload help function to work
    for ii = 1:numel(objs)
        if strncmp(class(objs{ii}), 'py.', 3) && ~is_simple_python(class(objs{ii}))
            objs{ii} = brille.generic_python_wrapper(objs{ii});
        end
    end
end
    ...
    while ii <= numel(s)
        if s(ii).type == '.'
            if numel(s) > ii && strcmp(s(ii+1).type, '()')
                args = brille.light_python_wrapper.parse_args(s(ii+1).subs, py.getattr(varargout, s(ii).subs));
        ...
        elseif strcmp(s(ii).type, '{}')
            ...
            if numel(s) > ii && strcmp(s(ii+1).type, '()')
                args = brille.light_python_wrapper.parse_args(s(ii+1).subs, attribute);
            ...
            error('Syntax error: brille wrapper indexing error');
        end
    end
    if ~iscell(varargout)
        varargout = {varargout};
